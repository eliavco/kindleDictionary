...
class Definition {
	...
	getTemplate(): string {
		const translations = Object.keys(this.definitions).map(def => {
			...
			`<li class="translation"><div class="partofspeech">${ def ? `${def}:` : '' }</div><p class="hebrew">${this.definitions[def].join(', ')}</p></li>`;
			return translationTemplate;
		}).join('');

		const template = `<idx:entry name="default" scriptable="yes" spell="yes"><div class="definition"><div class="origin"><h3 class="lookup"><idx:orth>${this.lookup}</idx:orth></h3><p class="pronounciation">/${this.pronounciation}/</p></div><ol>${translations}</ol></div></idx:entry><hr/>`;
		return template;
	}
}
